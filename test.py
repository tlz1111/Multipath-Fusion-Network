...
import time
#=========param setting=========
tr.cuda.set_device(0)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
torch.multiprocessing.set_start_method('spawn', force=True)
class_num=8
batch_size_test=1
#==========datpath=========
test_data_path = 'G:/TLZ/5nlos/1dataset/RT_7_3_p/test/'
check_point_directory = 'G:/TLZ/5nlos/2process/5ding/1main_84.72_1.32/'
#=========datset========
class myDataset(Dataset):
    def __init__(self,data_dir):
        self.label_name = {"bend": 0, "box": 1, "drink": 2,"jump":3,"squat":4,"step":5,"walk":6,"wave":7}
    ...
    def __getitem__(self, index):
        data_path,label=self.data_info[index]
        data=self.sample(data_path)
        return data,label

    def __len__(self):
        return len(self.data_info)

    def sample(self,matpath):
        image = Image.open(matpath)
        image = self.transform(image)
        image = np.array(image)
        c_mat=torch.tensor(image)
        return c_mat

    def get_data_info(self,datapath):
        data_info=list()
        dirs=os.listdir(datapath)
        for sub_dir in dirs:
            c_mat_names = os.listdir(os.path.join(datapath, sub_dir))
            for i in range(len(c_mat_names)):
                    c_mat_name = c_mat_names[i]
                    path_img = os.path.join(datapath, sub_dir, c_mat_name)
                    label = dict(self.label_name)[sub_dir]
                    data_info.append((path_img, int(label)))
        return data_info

test_data = myDataset(data_dir=test_data_path)
...
class Mulpath_fusion(nn.Module):
    def __init__(self, input_channels=3):
        ...
        self.cnn2_4 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, padding=3 // 2)
...
def test(check_path):
    model_ft.eval()
    eval_acc=0.
    model_ft.load_state_dict(tr.load(check_path))
    pred1=[]
    labels1=[]
    with torch.no_grad():
      for i, data in enumerate(test_loader):
        ...
        # forward
        out = model_ft(inputs,batch_size_test)
        _, pred = tr.max(out, 1)
        num_correct = (pred == labels).sum()
        eval_acc += num_correct.item()
        pred1.append(pred)
        labels1.append(labels)
    pred1=torch.stack(pred1).detach().cpu()
    labels1 = torch.stack(labels1).detach().cpu()
    print('Final Test Acc: {:.6f}'.format(eval_acc / (len(test_data))))
    return eval_acc / (len(test_data)),pred1,labels1
...
if __name__ == '__main__':
    ...
    for ij in range(5):
        ...
        check_path = check_point_directory + str(ij + 1) + '/' + 'end_epoch' + '.pkl'
        test_acc, pred, label = test(check_path)
        test_total_acc.append(test_acc)
    avg=np.mean((test_total_acc))
    print('Avg:',avg)
    std = np.std((test_total_acc))
    print('Std:', std)
